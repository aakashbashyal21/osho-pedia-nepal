...
import { getBlogList, getRecentMeditationListCard } from "@/data/action";

let page = 2;

export type BlogArtwork = JSX.Element;

function LoadMoreMeditation() {
    ...
    useEffect(() => {
        if (inView && hasMoreData) { // Check if there is more data to load
            ...
            const timeoutId = setTimeout(() => {
                getRecentMeditationListCard(page).then((res) => {
                    if (res.length > 0) {
                        setData([...data, ...res]);
                        page++;
                    } else {
    ...
    return (
        <>
            <section className="grid grid-cols-1 xl:grid-cols-2 lg:grid-cols-1 gap-6 lg:gap-8">
                {data}
            </section>
...
export default LoadMoreMeditation;
